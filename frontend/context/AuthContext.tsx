...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const uploadBusinessLogo = async (file: File) => {
        const response = await auth.uploadBusinessLogo(file);
        setUser(response.data);
    };

    return (
        <AuthContext.Provider value={{ user, loading, login, register, logout, updateProfile, uploadAvatar, uploadBusinessLogo }}>
            {children}
